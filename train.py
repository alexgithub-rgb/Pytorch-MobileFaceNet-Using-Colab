...
add_arg('batch_size',       int,    20,                       '训练的批量大小')
add_arg('num_workers',      int,    0,                        '读取数据的线程数量')
add_arg('num_epoch',        int,    30,                       '训练的轮数')
add_arg('learning_rate',    float,  0.001,                     '初始学习率的大小')
add_arg('train_root_path',  str,    '/content/gdrive/My Drive/Colab Notebooks/face/Pytorch-MobileFaceNet/dataset/train_data10',     '训练数据的根目录')
add_arg('test_list_path',   str,    '/content/gdrive/My Drive/Colab Notebooks/face/Pytorch-MobileFaceNet/dataset/lfw_test/lfw_test.txt',   '测试数据的数据列表路径')
add_arg('save_model',       str,    'save_model/new10',            '模型保存的路径')
