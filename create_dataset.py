...
# 人脸识别训练数据的格式转换
def convert_data(root_path, output_prefix,partial):
    # 读取全部的数据类别获取数据
    person_id = 0
    data = []
    ...
    for image_path, person_id in tqdm(data):
        try:
            ...
            # img = (np.sqrt (img)*2).astype(np.uint8)
            _, img = cv2.imencode('.bmp', img)
            # 写入对应的数据
            writer.add_img(key, img.tobytes())
...
if __name__ == '__main__':
    ...
    # load_mx_rec(Path("/content/gdrive/My Drive/Colab Notebooks/face/faces_emore"), Path("/content/gdrive/My Drive/Colab Notebooks/face/faces_emore/"))
    convert_data(Path("/content/gdrive/My Drive/Colab Notebooks/face/Pytorch-MobileFaceNet/dataset/lfw-deepfunneled"), Path("/content/gdrive/My Drive/Colab Notebooks/face/Pytorch-MobileFaceNet/dataset/train_data25"),25)
